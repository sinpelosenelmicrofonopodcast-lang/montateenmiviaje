...
export async function POST(request: Request) {
  try {
    ...
    const raffleId = String(formData.get("raffleId") ?? "raffle");
    ...
    if (file.size <= 0 || file.size > MAX_FILE_BYTES) {
      return NextResponse.json({ message: "El comprobante excede el límite permitido (8MB)" }, { status: 400 });
    }

    await ensureBucketExists();

    const safeRaffleId = raffleId.replace(/[^a-zA-Z0-9-_]/g, "").slice(0, 80) || "raffle";
    const ext = extensionFromName(file.name);
    const filePath = `raffles/${safeRaffleId}/proofs/${Date.now()}-${randomUUID()}.${ext}`;
    ...
    if (upload.error) {
      return NextResponse.json({ message: `No se pudo subir comprobante: ${upload.error.message}` }, { status: 500 });
    }
